        ...
        public async Task<BaseDataCollection<DiscountCodeEntity>> GetListDiscountCode(DataGridModel model)
        {
            ...
            if (condition.FilterCondition?.Any() == true)
            {
                foreach (var con in condition.FilterCondition)
                {
                    ...
                    {
                        query = query.Where(x => con.Values.Select(a => a).Contains(x.DiscountCodeNumber));
                    }
                    if (con.ColumnName == "Status" && con.Values?.Any() == true)
                    {
                        query = query.Where(x => con.Values.Select(a => (int?)a).Contains(x.Status));
                    }
                    if (con.ColumnName == "Date Range" && con.Values?.Any() == true)
                    {
                        DateTimeOffset.TryParse(con.Values[0].ToString(), out DateTimeOffset startDate);
                        DateTimeOffset.TryParse(con.Values[1].ToString(), out DateTimeOffset endDate);
                        var start = startDate.AddHours(8).Date;
                        var end = endDate.AddHours(8).Date;
                        query = query.Where(x => x.StartTime != null && x.EndTime != null
                                                                       && (x.StartTime.Value.AddHours(8).Date >=
                                                                           start &&
                                                                           x.EndTime.Value.AddHours(8).Date <= end));
                    }
                }
            }
            ...
            var listDiscountCode = await _context.DiscountCodeEntities.OrderByDescending(d => d.CreateAt).ToListAsync();
            var countTotal = await _context.DiscountCodeEntities.CountAsync();
